...
export default function GlobalSettingsPage(props) {
 const [globalSettings, setGlobalSettings] = createSignal(null); // Start with null to indicate loading
 const selectedPart = () =>
  props.settingsPart.replace("global-", "") || "display"; // Provide a default fallback value
  ...
  try {
   ...
   if (settingsToSave.installation_settings) {
    await invoke("change_installation_settings", {
     settings: settingsToSave.installation_settings,
    });
   }

   if (settingsToSave.display) {
    await invoke("change_gamehub_settings", {
     settings: settingsToSave.display,
    });
   }
  ...
  switch (selectedPart()) {
   case "display":
    try {
     ...
     await message("Display settings got reseted to default successfully !", {
      title: "FitLauncher",
      kind: "info",
     });
    } catch (error) {
   ...
   case "dns":
    try {
     ...
     await message("DNS settings got reseted to default successfully !", {
      title: "FitLauncher",
      kind: "info",
     });
    } catch (error) {
   ...
   case "install":
    try {
     ...
     await message(
      "Installation settings got reseted to default successfully !",
      { title: "FitLauncher", kind: "info" },
     );
    } catch (error) {
     let formatted_message =
      "Error resetting Installation settings ! :" + error;
     await message(formatted_message, { title: "FitLauncher", kind: "error" });
    }
    break;
  }
...
function DisplayPart({ settings, handleSwitchCheckChange }) {
 ...
 onMount(async () => {
  try {
   ...
   if (defaultThemeKeys.includes(savedTheme)) {
    ...
    setCurrentTheme(
     savedTheme.replace(/-/g, " ").replace(/\b\w/g, char => char.toUpperCase()),
    );
 ...
 async function revertToDefault() {
  ...
  setCurrentTheme(
   defaultTheme.replace(/-/g, " ").replace(/\b\w/g, char => char.toUpperCase()),
  );
 ...
 async function handleAddTheme() {
  ...
  try {
   ...
   if (!match) {
    await message("Invalid theme block structure.", {
     title: "FitLauncher",
     kind: "error",
    });
    return;
   ...
   const isValidTheme = requiredVariables.every(variable =>
   ...
   if (isValidTheme) {
    ...
    setNewThemes([
     ...newThemes(),
     themeName.replace(/-/g, " ").replace(/\b\w/g, char => char.toUpperCase()),
    ]);
   ...
   <div class="global-page-group" id="global-display">
    ...
    <ul class="global-page-group-list">
     ...
     <li>
      ...
      <label class="switch">
       <input
        ...
        onchange={() =>
         handleSwitchCheckChange("display.auto_get_colors_popular_games")
        }
       />
       <span class="switch-slider round"></span>
      </label>
     ...
     <li>
      ...
      />
      <button
       class="plus-button-settings"
       onclick={async () => await handleAddTheme()}
      >
       <span>+</span>
      </button>
     </li>
    ...
    <ul class="global-page-group-list">
     <li>
      <span>
       Enable Auto-Install{" "}
       <small>
        <i>(Will install the game directly after downloading)</i>
       </small>{" "}
       :
      </span>
      <label class="switch">
       <input
        ...
        onchange={() =>
         handleSwitchCheckChange("installation_settings.auto_install")
        }
       />
       <span class="switch-slider round"></span>
      </label>
     ...
     <li>
      <span>
       ...
       <small>
        <i>
         (Will clean the game's setup after installing and introducing the path
         in library)
        </i>
       </small>{" "}
       :
      ...
      <label class="switch">
       <input
        ...
        onchange={() =>
         handleSwitchCheckChange("installation_settings.auto_clean")
        }
       />
       <span class="switch-slider round"></span>
      </label>
     ...
     <li>
      <span>
       Enable 2GB Limit for installation{" "}
       <small>
        <i>(Will be automatically enabled if you have 8GB or less)</i>
       </small>{" "}
       :
      </span>
      <label class="switch">
       <input
        ...
        onchange={() =>
         handleSwitchCheckChange("installation_settings.two_gb_limit")
        }
       />
       <span class="switch-slider round"></span>
      </label>
     ...
     <li>
      <span>
       Enable installation of DirectX{" "}
       <small>
        <i>
         (It's only useful once if you have never installed fitgirl repacks)
        </i>
       </small>{" "}
       :
      </span>
      <label class="switch">
       <input
        ...
        onchange={() =>
         handleSwitchCheckChange("installation_settings.directx_install")
        }
       />
       <span class="switch-slider round"></span>
      </label>
     ...
     <li>
      <span>
       Enable installation of Microsoft C++{" "}
       <small>
        <i>
         (It's only useful once if you have never installed fitgirl repacks)
        </i>
      ...
      <label class="switch">
       <input
        ...
        onchange={() =>
         handleSwitchCheckChange("installation_settings.microsoftcpp_install")
        }
       />
       <span class="switch-slider round"></span>
      </label>
...
function CacheSettings() {
 ...
 async function handleClearCache() {
  const confirmation = await confirm(
   "This will delete every cache files, this action cannot be reverted, are you sure ?",
 ...
 async function handleCheckUpdate() {
  ...
  if (!updateClicked()) {
   await message("Please wait before clicking again it can take some time.", {
    title: "FitLauncher",
    kind: "info",
   });
   ...
   if (update) {
    ...
    const confirm_update = await confirm(
    ...
    await message(
     `No update found, you are on the latest version ${current_ver}.`,
    );
    setUpdateClicked(false);
   }
   ...
   <ul class="global-page-group-list">
    <li>
     <span>
      ...
      <small>
       <i>
        (This will remove images cache and all torrents cache, dht and session
        data. )
       </i>
      </small>
      :
     ...
     <button
      class="clear-cache-settings-button"
      onclick={async () => {
    ...
    <li>
     <span>
      ...
      <small>
       <i>
        (Please do not share this with anyone except the official FitLauncher's
        Moderation !)
       </i>
      </small>
      :
     ...
     <button
      class="go-to-logs-settings-button"
      onclick={async () => {
       await handleGoToLogs();
      }}
     >
      <span>Go !</span>
     </button>
