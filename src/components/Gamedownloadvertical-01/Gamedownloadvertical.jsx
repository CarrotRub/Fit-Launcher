...
import {
  restartTorrentInfo,
  setTorrentTrigger,
} from "../functions/dataStoreGlobal";
...
function Gameverticaldownloadslide({ isActive, setIsActive }) {
  ...
  const [showDownloadData, setShowDownloadData] = createSignal(false);
    ...
    const iconCorr = () => {
      ...
      switch (state) {
        case "paused":
          return (
            <svg
              xmlns="http://www.w3.org/2000/svg" width="24"height="24"viewBox="0 0 24 24"fill="none" stroke="currentColor"stroke-width="2"stroke-linecap="round"stroke-linejoin="round" class="lucide lucide-circle-play"
            >
  ...
  const handleStopTorrent = async () => {
    ...
    if (CTG) {
      Swal.fire({
        ...
        footer:
          'You can also delete the files directly by clicking on this button <button id="delete-files-btn" class="swal2-styled" style="background-color: red; color: white;">Delete Files</button>!',
        icon: "warning",
            ...
            deleteFilesBtn.addEventListener("click", async () => {
              try {
                  ...
                  if (
                    ...
                    try {
                      ...
                      try {
                        ...
                        console.error("Error Deleting Torrent Again:", error);
                        unexpectedGameResumeError();
                      }
                    } catch (err) {
                ...
                Swal.fire({
                  ...
                  footer:
                    "If you do not understand the error, please contact us on Discord before opening any issues on GitHub.",
                  icon: "error",
                });
              }
            });
        ...
        if (result.isConfirmed) {
          ...
          try {
            ...
            console.error("Error Resuming Torrent :", error);
            if (error.AnyhowError === "TorrentManager is not initialized.") {
              const lastInputPath = localStorage.getItem("LUP");
              console.log(
                "TorrentManager is not initialized. Initializing it..."
              );
              ...
              try {
                ...
                try {
                  ...
                  console.error("Error Stopping Torrent Again:", error);
                  unexpectedGameResumeError();
                }
              } catch (err) {
  ...
  // Show or hide the download data
  const handleDownloadDataToggle = () => {
    setShowDownloadData(!showDownloadData());

    // initialize the charts if they are not already initialized
    if (showDownloadData()) {
      const ctx1 = document
        .getElementById("downloadUploadChart")
        .getContext("2d");
  ...
  return (
    <div class="sidebar-space" style={{ display: isActive ? "block" : "none" }}>
      ...
      <div class="stats-panel">
        <h2 class="game-download-title">Game Download Progress</h2>
        ...
        <div class="show-download-container">
          <button class="show-download-btn" onClick={handleDownloadDataToggle}>
            Show Download Data
          </button>
        </div>
          ...
          <>
            ...
            <div class="peer-stats-container">
              ...
              {peerStats() ? (
                <>
                  <p>
                    <strong>Connected Peers:</strong> {peerStats().live}
                  </p>
                  <p>
                    <strong>Connecting Peers:</strong> {peerStats().connecting}
                  </p>
                  <p>
                    <strong>Dead Peers:</strong> {peerStats().dead}
                  </p>
                  <p>
                    <strong>Queued Peers:</strong> {peerStats().queued}
                  </p>
                  <p>
                ...
                <p class="peer-stats-desc">
                  No peer data available as there is nothing downloading.
                </p>
              )}
            </div>
          </>
        )}
        <p class="issues-with-download-desc">
          Having issues with downloading games?
        </p>
      </div>
    </div>
  );
