...
pub async fn refresh_discovery_games(app: AppHandle) -> Result<(), ScrapingError> {
    ...
    let mut fetcher = WpGameFetcher::new();
    fetcher.fetch_discovery_games().await?;

    let mut have: HashSet<String> = queue.iter().map(|g| g.title.clone()).collect();

    let mut fresh: Vec<Game> = fetcher
        .discovery_games
        .into_iter()
        .filter(|g| g.img.contains("imageban") && have.insert(g.title.clone()))
        .collect();
    ...
    db::set_category_games(&conn, "discovery", &queue, hash_url)?;
    info!("wrote discovery pool, queue size now {}", queue.len());
